    ...
    public function modifyAdminMenu()
    {
        if (auth()->user()->can('superadmin')) {
            Menu::modify(
                'admin-sidebar-menu',
                function ($menu) {
                    $menu->url(action('\Modules\Superadmin\Http\Controllers\SuperadminController@index'), __('superadmin::lang.superadmin'), ['icon' => 'superadminimg', 'active' => request()->segment(1) == 'superadmin'])->order(1);
                }
            );
        }
        ...
        if (auth()->user()->can('superadmin.access_package_subscriptions') && auth()->user()->can('business_settings.access')) {
            ...
            $menu->whereTitle(__('business.settings'), function ($sub) {
                $sub->url(
                    ...
                    ['icon' => 'superadminimg', 'active' => request()->segment(1) == 'subscription']
                );
            });
        }
    }
