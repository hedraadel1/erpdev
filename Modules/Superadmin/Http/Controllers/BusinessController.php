...
class BusinessController extends BaseController
{
  ...
  public function index()
  {
    ...
    if (request()->ajax()) {
      ...
      $businesses = Business::leftjoin('subscriptions AS s', function ($join) use ($date_today) {
        ...
        ->leftjoin('business_categories as bc', 'business.category_id', '=', 'bc.id')
        ->leftjoin('wallet as w', 'w.business_id', '=', 'business.id')
      ...
      if ($is_old == 'is_old_yes') {
        $businesses->where('business.is_old', 1);
      } else if ($is_old == 'is_old_no') {
        $businesses->where('business.is_old', 0);
      }
      ...
      return Datatables::of($query)
        ...
        ->editColumn('is_old', '@if($is_old == 1) <span class="label bg-red">@lang("lang_v1.isold_yes")</span> @else <span class="label bg-blue">@lang("lang_v1.isold_no")</span> @endif')
        ...
        ->addColumn('actions', function ($row) {
          ...
          $html = '<a href="' .
            action("\Modules\Superadmin\Http\Controllers\BusinessController@show", [$row->id]) . '"
                            style="width:150px;margin-bottom: 5px;" class="btn btn-info Btn-Brand btn-xs">' . __('superadmin::lang.manage') . '</a>
                 
                            <a style="width:150px;margin-bottom: 5px;" class="btn btn-xs Btn-Brand Btn-Primary " href="' . action('\Modules\Superadmin\Http\Controllers\SuperadminProductController@showBusinessProduct', $row->id) . '" >'
            . 'المنتجات ' . '</a>';

          if ($row->is_active == 1) {
            $html .= ' <a href="' . action('\Modules\Superadmin\Http\Controllers\BusinessController@toggleActive', [$row->id, 0]) . '"
                                style="width:150px;margin-bottom: 5px;" class="btn btn-danger btn-xs Btn-Brand link_confirmation">' . __('lang_v1.deactivate') . '
                                </a>';
          } else {
            $html .= ' <a href="' . action('\Modules\Superadmin\Http\Controllers\BusinessController@toggleActive', [$row->id, 1]) . '"
                                style="width:150px;margin-bottom: 5px;" class="btn btn-success btn-xs Btn-Brand link_confirmation">' . __('lang_v1.activate') . '
                                </a>';
          }
          if ($row->is_old == 1) {
            $html .= ' <a style="width:150px;margin-bottom: 5px;" href="' . action('\Modules\Superadmin\Http\Controllers\BusinessController@toggleISold', [$row->id, 0]) . '"
                                    class="btn btn-success btn-xs Btn-Brand link_confirmation">' . __('lang_v1.isold_no') . '
                                </a>';
          } else {
            $html .= ' <a style="width:150px;margin-bottom: 5px;"  href="' . action('\Modules\Superadmin\Http\Controllers\BusinessController@toggleISold', [$row->id, 1]) . '"
                                    class="btn btn-danger btn-xs Btn-Brand link_confirmation">' . __('lang_v1.isold_yes') . '
                                </a>';
          }
          if (request()->session()->get('user.business_id') != $row->id) {
            $html .= ' <a style="width:150px;margin-bottom: 5px;"  href="' . action('\Modules\Superadmin\Http\Controllers\BusinessController@destroy', [$row->id]) . '"
                                    class="btn btn-success btn-xs Btn-Brand delete_business_confirmation">' . __('messages.delete') . '</a>';
          }
  ...
  public function create()
  {
    ...
    $categories = BusinessCategory::latest()->pluck('name', 'id')->toArray();
  ...
  public function store(Request $request)
  {
    ...
    try {
      ...
      $business_details = $request->only(['name', 'category_id', 'start_date', 'currency_id', 'tax_label_1', 'tax_number_1', 'tax_label_2', 'tax_number_2', 'time_zone', 'accounting_method', 'fy_start_month']);
  ...
  public function toggleISold(Request $request, $business_id, $is_old)
  {
    if (!auth()->user()->can('superadmin')) {
      abort(403, 'Unauthorized action.');
    }
  ...
  public function toggleIsPide(Request $request)
  {
    $inputs = $request->all();
    $business = Business::where('id', $inputs['business_id'])->first();
    $business->is_paid = $inputs['is_paid'];
    $business->save();
    $output = [
      'success' => 1,
      'msg' => 'تم تعديل النشاط بنجاح'
    ];
    return $output;
  }
}