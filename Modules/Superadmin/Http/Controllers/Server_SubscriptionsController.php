...
use Modules\Superadmin\Entities\Server_Subscriptions;
...
class Server_SubscriptionsController extends BaseController
{
  protected $provider;
  ...
  public function index()
  {
    ...
    //Get active Server_Subscriptions and upcoming subscriptions.
    $active = Server_Subscriptions::active_server_subscriptions($business_id);

    $nexts = null;
    $waiting = Server_Subscriptions::waiting_approval($business_id);

    $servertypes = ServerType::active()->orderby('server_name')->get();
    ...
    return view('superadmin::superadmin_subscription.index')
      ->with(compact('wallet', 'servertypes', 'active', 'intervals'));
  }

  /**
   * Show pay form for a new ServerType.
   * @return Response
   */
  public function pay($ServerType_id, $form_register = null)
  {
    if (!auth()->user()->can('superadmin.access_package_subscriptions')) {
      abort(403, 'Unauthorized action.');
    }
    ...
    try {
      ...
      $servertype = ServerType::active()->find($ServerType_id);

      //Check for free ServerType & subscribe it.
      if ($servertype->server_price_perday == 0) {
        $gateway = null;
        $payment_transaction_id = 'FREE';
        $user_id = request()->session()->get('user.id');

        $this->_add_subscription($business_id, $servertype, $gateway, $payment_transaction_id, $user_id);
      ...
      return view('superadmin::Server_Subscriptions.pay')
        ->with(compact('ServerType', 'gateways', 'system_currency', 'layout', 'user', 'offline_payment_details'));
    } catch (\Exception $e) {
      DB::rollBack();
  ...
  /**
   * Save the payment details and add Server_Subscriptions details
   * @return Response
   */
  public function confirm($package_id, Request $request)
  {
    ...
    try {
      ...
      $user_id = request()->session()->get('user.id');
      $ServerType = ServerType::active()->find($package_id);
      ...
      if (method_exists($this, $pay_function)) {
        $payment_transaction_id = $this->$pay_function($business_id, $business_name, $ServerType, $request);
      }

      //Add Server_Subscriptions details after payment is succesful
      $Server_Subscriptions = $this->_add_subscription($business_id, $package_id, request()->gateway, $payment_transaction_id, $user_id);
      ...
      if ($Server_Subscriptions == false) {
        $output = ['success' => 0, 'msg' => 'رصيد المحفظه لايكفي'];
      }
    } catch (\Exception $e) {
      DB::rollBack();
  ...
  protected function confirm_pesapal($transaction_id, $request)
  {
    ...
    if ($pesapal_session['ref'] == $merchant_reference) {
      ...
      $this->_add_subscription($business_id, $ServerType, 'pesapal', $transaction_id, $user_id);
   ...
   * @return Response
   */
  protected function pay_stripe($business_id, $business_name, $ServerType, $request)
  {
    Stripe::setApiKey(env('STRIPE_SECRET_KEY'));

    $metadata = ['business_id' => $business_id, 'business_name' => $business_name, 'stripe_email' => $request->stripeEmail, 'package_name' => $ServerType->name];
   ...
   * @return Response
   */
  protected function pay_offline($business_id, $business_name, $ServerType, $request)
  {
    ...
    $system_currency = System::getCurrency();
    $ServerType->price = $system_currency->symbol . number_format($ServerType->price, 2, $system_currency->decimal_separator, $system_currency->thousand_separator);

    Notification::route('mail', $email)
      ->notify(new SubscriptionOfflinePaymentActivationConfirmation($business, $ServerType));
   ...
   * @return Response
   */
  protected function pay_paypal($business_id, $business_name, $ServerType, $request)
  {
    ...
    $data['items'] = [
      [
        ...
        'price' => (float)$ServerType->price,
        'qty' => 1
      ]
    ];
    $data['invoice_id'] = $invoice_id;
    $data['invoice_description'] = "Order #{$data['invoice_id']} Invoice";
    $data['return_url'] = action('\Modules\Superadmin\Http\Controllers\SubscriptionController@confirm', [$ServerType->id]);
    $data['cancel_url'] = action('\Modules\Superadmin\Http\Controllers\SubscriptionController@pay', [$ServerType->id]);
    $data['total'] = (float)$ServerType->price;
  ...
  public function paypalExpressCheckout(Request $request, $package_id)
  {
    ...
    // Get the cart data or ServerType details.
    $ServerType = ServerType::active()->find($package_id);
   ...
   * @return Response
   */
  protected function pay_razorpay($business_id, $business_name, $ServerType, $request)
  {
    $razorpay_payment_id = $request->razorpay_payment_id;
    $razorpay_api = new Api(env('RAZORPAY_KEY_ID'), env('RAZORPAY_KEY_SECRET'));

    $payment = $razorpay_api->payment->fetch($razorpay_payment_id)->capture(['amount' => $ServerType->price * 100]); // Captures a payment
  ...
  public function show($id)
  {
    ...
    $Server_Subscriptions = Server_Subscriptions::where('business_id', $business_id)
      ->with(['ServerType', 'created_user', 'business'])
      ->find($id);
    ...
    return view('superadmin::Server_Subscriptions.show_subscription_modal')
      ->with(compact('Server_Subscriptions', 'system'));
  }
  ...
  public function allSubscriptions()
  {
    ...
    $subscriptions = Server_Subscriptions::where('server_subscriptions.business_id', $business_id)
      ->leftjoin(
        'server_types as P',
        'server_subscriptions.server_types_id',
        '=',
        'P.id'
      )
      ->leftjoin(
        'users as U',
        'server_subscriptions.created_id',
        '=',
        'U.id'
      )
      ->addSelect(
        'P.server_name as server_name',
    ...
    return Datatables::of($subscriptions)
      ...
      ->editColumn(
        'server_types_price',
        '<span class="display_currency" data-currency_symbol="true">{{$server_types_price}}</span>'
      )
      ->editColumn(
        'created_at',
      ...
      ->addColumn('action', function ($row) {
        return '<button type="button" class="btn btn-primary btn-xs btn-modal" data-container=".view_modal" data-href="' . action("\Modules\Superadmin\Http\Controllers\Server_SubscriptionsController@show", $row->id) . '" ><i class="fa fa-eye" aria-hidden="true"></i> ' . __("messages.view") . '</button>';
      })
      ->rawColumns(['server_types_price', 'action'])
      ->make(true);
  }
}