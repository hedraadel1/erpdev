            ...
            return Datatables::of($query)
                ...
                ->addColumn('action', function($row) {
                    ...
                    $html = '<a href="' .
							...
							if($row->is_old == 1) {
                                $html .= ' <a href="' . action('\Modules\Superadmin\Http\Controllers\BusinessController@toggleISold', [$row->id, 0]) . '"
                                    class="btn btn-success btn-xs link_confirmation">' . __('lang_v1.isold_no') . '
                                </a>';
                            } else {
                                $html .= ' <a href="' . action('\Modules\Superadmin\Http\Controllers\BusinessController@toggleISold', [$row->id, 1]) . '"
                                    class="btn btn-danger btn-xs link_confirmation">' . __('lang_v1.isold_yes' ) . '
                                </a>';
                            }
                            if(request()->session()->get('user.business_id') != $row->id) {
                                $html .= ' <a href="' . action('\Modules\Superadmin\Http\Controllers\BusinessController@destroy', [$row->id]) . '"
                                    class="btn btn-success btn-xs delete_business_confirmation">' . __('messages.delete' ) . '</a>';
                            }
                ...
                ->rawColumns(['action', 'is_active','is_old',  'created_at'])
                ->make(true);
        }
