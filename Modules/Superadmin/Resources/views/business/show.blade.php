    ...
    <section class="content">
        <div class="box box-solid">
            <div class="box-header">
                ...
                <div class="paid" style="float:left">
                    <input type="checkbox" {{ $business->is_paid == 1 ? 'checked' : '' }} name="is_paid" class="input-check"
                        id="business_is_paid" data-business_id="{{ $business->id }}" value="1">
                    <label for="business_is_paid">عميل متعاقد</label>
                </div>
            </div>
                    ...
                    <div class="col-sm-3">
                        <div class="well well-sm">
                            ...
                            @if (!empty($business->owner))
                                <p class="text-muted">
                                    {{ optional($business->owner)->surname }} {{ optional($business->owner)->first_name }}
                                    {{ optional($business->owner)->last_name }}
                                </p>
                            @endif
                            ...
                            <p class="text-muted">
                                {{ optional($business->owner)->contact_no }}
                            </p>

                            <strong><i class="fa fa-map-marker margin-r-5"></i>
                                @lang('business.address')</strong>
                            <p class="text-muted">
                                {{ optional($business->owner)->address }}
                            </p>
                        </div>
                    </div>
...
@section('javascript')
    <script type="text/javascript">
        ...
        $('#business_is_paid').on('click', function() {
            var is_paid = 0;
            ...
            $.ajax({
                url: '/superadmin/business/toggle-is_paid',
                dataType: 'json',
                method: 'POST',
                data: {
                    'is_paid': is_paid,
