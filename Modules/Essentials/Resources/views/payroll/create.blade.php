                        ...
                        @foreach($payrolls as $employee => $payroll)
                            @php
                              /*   if ($action != 'edit') {
                                ...
                                if ($action != 'edit') {
                                    ...
                                    $total_work_duration = 1;
                                    $duration_unit = __('lang_v1.salarybyhour');
                                    if ($payroll['essentials_pay_period'] == 'week') {
                                        $total_work_duration = 4;
                                        $duration_unit = __('essentials::lang.salarybyhour');
                                    } elseif ($payroll['essentials_pay_period'] == 'day') {
                                        $total_work_duration = \Carbon::parse($transaction_date)->daysInMonth;
                                        $duration_unit = __('lang_v1.salarybyhour');
                                    }
                                    elseif ($payroll['essentials_pay_period'] == 'month') {
                                        $total_work_duration = \Carbon::parse($transaction_date)->daysInMonth;
                                        $duration_unit = __('lang_v1.salarybyhour');
                                    }
                                    $totalbyhour = $payroll['total_work_duration'];
                                    $total = $total_work_duration * $totalbyhour;
                                } else {
                                    $amount_per_unit_duration = $payroll['essentials_amount_per_unit_duration'];
                                    $total_work_duration = $payroll['essentials_duration'];
                                    $duration_unit = $payroll['essentials_duration_unit'];
                            ...
                            <tr data-id="{{$employee}}">
                                ...
                                <td>
                                    ...
                                    {!! Form::text('payrolls['.$employee.'][essentials_duration]', $payroll['total_work_duration'], ['class' => 'form-control input_number essentials_duration', 'placeholder' => __( 'essentials::lang.total_work_duration' ), 'required', 'data-id' => $employee, 'id' => 'essentials_duration_'.$employee]); !!}
                                    <br>
