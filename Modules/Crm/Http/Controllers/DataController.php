        ...
        if ($is_crm_enabled) {
            Menu::modify(
                ...
                function ($menu) use ($is_admin) {
                    $menu->url(action('\Modules\Crm\Http\Controllers\CrmDashboardController@index'), __('crm::lang.crm'), ['icon' => 'crmimg', 'active' => request()->segment(1) == 'crm' || request()->get('type') == 'life_stage' || request()->get('type') == 'source'])->order(86);
                }
            );
           ...
           /*  $business = Business::find(auth()->user()->business_id);
            ...
            if (!empty($crm_settings['enable_order_request']) && auth()->user()->can('sell.create')) {
