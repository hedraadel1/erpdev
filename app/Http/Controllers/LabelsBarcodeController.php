...
class LabelsBarcodeController extends Controller
{
    /**
     * All Utils instance.
     *
    ...
    public function show(Request $request)
    {
        ...
        $barcode_settings = Barcode::where('business_id', $business_id)->where('name', 'defualt')->first()
                           ??  Barcode::create([
                            'name'=>'defualt',
                            'width'=>'1.54',
                            'height'=>'0.97',
        ...
        return view('labels_barcode.show')
            ->with(compact('products', 'barcode_settings',  'price_groups'));
    }
    ...
    public function preview(Request $request)
    {
          ...
          // $barcode_setting['stickers_in_one_sheet'] ='1';

            $barcode_details = Barcode::where('name', 'defualt')->first();
            if ($barcode_details) {
              $barcode_details->update($barcode_setting);
            }
            ...
            foreach ($product_details_page_wise as $page => $page_products) {
                ...
                $output = view('labels_barcode.partials.preview_2')
