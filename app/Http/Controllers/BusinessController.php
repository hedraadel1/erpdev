...
class BusinessController extends Controller
{
    ...
    */
	...
	public function getbasicBusinessSettings()
    {
		return view('business.basicsetting');
	}
    ...
    public function getBusinessSettings()
    {
        ...
        $whastapp_setting['days'] = StaticsMsg::$DAY;
        $whastapp_setting['type'] = StaticsMsg::$TYPE;
        $whastapp_setting['actions'] = StaticsMsg::$ACTIONS;
        $whastapp_setting['all_action'] = StaticsMsg::where('business_id',$business_id)->get();
        $whastapp_setting['auto_action_msgs'] = AutoActionMsg::where('business_id',$business_id)->get();
       
        return view('business.settings', compact('whastapp_setting','business', 'currencies', 'tax_rates', 'timezone_list', 'months', 'accounting_methods', 'commission_agent_dropdown', 'units_dropdown', 'date_formats', 'shortcuts', 'pos_settings', 'modules', 'theme_colors', 'email_settings', 'sms_settings', 'mail_drivers', 'allow_superadmin_email_settings', 'custom_labels', 'common_settings', 'weighing_scale_setting', 'payment_types'));
    }
    ...
    public function postBusinessSettings(Request $request)
    {
        ...
        try {
            ...
            $business_details = $request->only(['name', 'start_date', 'currency_id', 'tax_label_1', 'tax_number_1', 'tax_label_2', 'tax_number_2', 'default_profit_percent', 'default_sales_tax', 'default_sales_discount', 'sell_price_tax', 'sku_prefix', 'time_zone', 'fy_start_month', 'accounting_method', 'transaction_edit_days', 'sales_cmsn_agnt', 'item_addition_method', 'currency_symbol_placement', 'on_product_expiry',
                ...
                'redeem_amount_per_unit_rp','setting_appkey','setting_authkey', 'min_order_total_for_redeem',
            ...
            if($request->whatsapp_setting){
                foreach ($request->whatsapp_setting as $key => $value) {
                  if ($value['action_time']!= null) {
                    // dd($key);
                    $data = [
                      'action_name' => $key, 
                      'action_time' => $value['action_time'], 
                      'action_duration' => $value['action_duration'], 
                      'action_day' => isset($value['action_day'])?$value['action_day'] : null, 
                      'business_id' => session('business.id'), 
                    ];
                    $action = StaticsMsg::where('action_name',$key)->first();
                    if ($action){
                     $action->update($data);
                    }else{
                      StaticsMsg::create($data);
                    }
                  }
                }
              }
              if($request->auto_action){
                // dd($request->auto_action);
                AutoActionMsg::where('business_id',$business_id)->delete();
  
                foreach ($request->auto_action as $key => $value) {
                    // dd($key);
                    $data = [
                      'action_name' => $key, 
                      'action_value' => $value, 
                    ...
                    AutoActionMsg::create($data);
                   
                }
                // dd($request->whatsapp_setting);
              }
